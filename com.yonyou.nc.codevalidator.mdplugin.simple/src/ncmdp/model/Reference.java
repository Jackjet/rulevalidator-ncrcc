...
public class Reference extends Cell {
	...
	public Cell getReferencedCell() {
		if (referencedCell == null && moduleName != null && mdFilePath != null && refId != null) {
	...
	public File getReferencedCellSourceFile(JGraph graph) {
		File file = NCMDPTool.getMDPFileByModuleAndPath(this.moduleName, this.mdFilePath);
		if (file != null) {
			HashMap<String, String> map = graph.getMap();
			String path = (String) map.get(refId);
	...
	public File getReferencedCellSourceFile() {
		...
		// return StringUtil.isEmptyWithTrim(filePath) ? null : new
		// File(filePath);
		try {
			RuntimeContext currentRuntimeContext = ExecutorContextHelperFactory.getExecutorContextHelper()
					.getCurrentRuntimeContext();
			BusinessComponent currentExecuteBusinessComponent = currentRuntimeContext
					.getCurrentExecuteBusinessComponent();
			String businessComponentPath = currentExecuteBusinessComponent.getBusinessComponentPath();
			String filePath = String.format("%s/METADATA/%s", businessComponentPath, mdFilePath);
			File result = new File(filePath);
			if (result.exists()) {
				return result;
			}
			// 在前期的执行阶段可能会导致没有设置NCHome
			String ncHome = currentRuntimeContext.getNcHome();
			if (StringUtils.isBlank(ncHome)) {
				return null;
			}
			filePath = String.format("%s/modules/%s/METADATA/%s", ncHome, moduleName, mdFilePath);
			File mdFile = new File(filePath);
			if (mdFile.exists()) {
				return mdFile;
			}
			String mdFileName = mdFilePath.substring(mdFilePath.lastIndexOf("\\") + 1);
			String mdFilePath = ReferenceMetadataManager.getInstance().findReferenceFilePath(ncHome, mdFileName);
			if (StringUtils.isBlank(mdFilePath)) {
				Logger.error(String.format("当前环境中不存在名为%s的文件，不能定位其路径!", mdFileName));
				return null;
			} else {
				return new File(mdFilePath);
			}
			//
			// @SuppressWarnings("unchecked")
			// Collection<File> listFiles = FileUtils.listFiles(new
			// File(ncHome+"/modules/"), new NameFileFilter(mdFileName),
			// DirectoryFileFilter.INSTANCE);
			// if(listFiles.size() > 0) {
			// //mazhqa: 可能同名的元数据文件在nchome中，如何定位，当前只去取第一个文件处理
			// return new ArrayList<File>(listFiles).get(0);
			// }
		} catch (RuleBaseException e) {
			Logger.error(String.format("Can't get module :%s md file(%s)!", moduleName, mdFilePath));
			return null;
		}
	}
	...
	// pds[0] = new NotEditableTextPropertyDescriptor(PROP_MODULE_NAME,
	// Messages.Reference_24);
	// pds[1] = new NotEditableTextPropertyDescriptor(PROP_MD_FILEPATH,
	// Messages.Reference_25);
	// pds[2] = new NotEditableTextPropertyDescriptor(PROP_REF_ID,
	// Messages.Reference_26);
	// return pds;
	// }
