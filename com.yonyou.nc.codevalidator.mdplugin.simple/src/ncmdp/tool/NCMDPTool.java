...
public class NCMDPTool {
	...
	// private static final String Cell_PROPTABLE_COL_SET =
	// "Cell_PROPTABLE_COL_SET";
	...
	public static synchronized DBConnProp[] getDBConnProps() {
		if (dbConnProps == null) {
			...
			if (dbConnProps == null || dbConnProps.length == 0) {
				...
				dbConnProps[0].setDbDriver("com.microsoft.sqlserver.jdbc.SQLServerDriver");
				dbConnProps[0]
	...
	public static String getCellFullClassName(JGraph graph, Cell cell) {
		...
		sb.append(".").append(componentName.toLowerCase()).append(".").append(cellName);
		return sb.toString();
	}
	...
	public static List<Accessor> getUseableAccessors(List<StereoType> stereoList) {
		...
		if (stereoList.size() > 1) {
			...
			for (int i = 1; i < stereoList.size(); i++) {
				...
				List<Accessor> accessorTempList = stereoList.get(i).getListAccessors();
	...
	public static synchronized String[] getAttrAccessors() {
		if (attrAccessors == null) {
			String attrAccessorFilePath = "/config/metadata/AttrAccessors.txt";
	...
	public static String getExceptionRecursionError(Throwable t) {
		...
		Throwable tt = t;
		while (tt != null) {
			sb.append("cause by ").append(tt.getClass().getName()).append(":").append(tt.getMessage()).append(";\r\n");
			tt = tt.getCause();
		}
		return sb.toString();
	}
	...
	// }
	//
	// public static void setPropTableSelectedColNames(ArrayList<ColSelObj>
	// cols) {
	// IStringVariableManager vvManager = VariablesPlugin.getDefault()
	// .getStringVariableManager();
	// IValueVariable cellpropcolset = vvManager
	...
	//
	/**
	 * 根据模块名和相对路径获得文件， 主要是用在Reference中查找引用的具体文件 mazhqa修改-仅从nchome中获取
	 * 
	 * @param moduleName
	 * @param relativePath
	 * @return
	 */
	public static File getMDPFileByModuleAndPath(String moduleName, String relativePath) {
		try {
			...
			String filePath = String.format("%s/METADATA/%s", businessComponentPath, relativePath);
			File result = new File(filePath);
			if (result.exists()) {
				return result;
			}
			String ncHome = currentRuntimeContext.getNcHome();
			filePath = ncHome + File.separator + "modules" + File.separator + moduleName + File.separator + "METADATA"
	 ...
	 * @param filter文件过滤器
	 */
	private static List<File> listFiles(List<File> filesList, File dir, FilenameFilter filter) {
