...
public class ClassLoaderUtilsImpl extends AbstractClassLoaderUtils {
	...
	protected ClassLoader getClassLoader(String projectPath) throws RuleClassLoadException {
		...
		synchronized (this) {
			...
			ClassLoader result = BootstrapClassLoader.getBootstrapClassLoader(ncHomeFolder, urlList.toArray(new URL[urlList.size()]));
			nameToLoaderMap.put(identifier, result);
			return result;
			// @SuppressWarnings("rawtypes")
			// Collection listFiles = FileUtils.listFiles(new
			// File(ncHomeFolder), new String[] { "jar" }, true);
			// for (Object fileObject : listFiles) {
			// File jarFile = (File) fileObject;
			// urlList.add(new URL(PROTOCAL_PREFIX +
			// computeForURLClassLoader(jarFile.getAbsolutePath())));
			// }
			// URLClassLoader result = new URLClassLoader(urlList.toArray(new
			// URL[0]));
		}
	}
	...
	private List<String> getProjectSourceUrls(String projectPath) {
		...
		BusinessComponent businessComponent = ProjectAnalyseUtils.getBusinessComponents(projectPath,
				GlobalExecuteUnit.DEFAULT_GLOBAL_NAME);
