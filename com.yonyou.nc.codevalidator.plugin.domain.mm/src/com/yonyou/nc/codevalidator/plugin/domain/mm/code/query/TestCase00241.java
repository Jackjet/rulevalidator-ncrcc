...
@RuleDefinition(
		...
		description = "部门参照需要按照组织过滤 ",
		relatedIssueId = "241",
		coder = "qiaoyanga",
		solution = "配置节点号参数，将通过节点号找到对应的配置文件，通过配置文件找到查询初试化类，判断类中是否有引用部门按组织过滤的类’nc.ui.mmf.busi.query.reffilter.QDeptFilter‘")
public class TestCase00241 extends AbstractXmlRuleDefinition {
	@Override
	...
	protected IRuleExecuteResult processScriptRules(IRuleExecuteContext ruleExecContext, List<XmlResource> resources)
		...
		for (XmlResource xmlResource : resources) {
			...
			try {
				...
				if (!isImport) {
					result.addResultElement(javaClassResource.getJavaCodeClassName(), "查询初试化类中部门参照未按照组织过滤 \n");
					continue;
				}
	...
	private static class QryCondDLGInitializer extends VoidVisitorAdapter<Void> {
		...
		public void visit(ImportDeclaration n, Void arg) {
			...
			if ("nc.ui.mmf.busi.query.reffilter.QDeptFilter".equals(importClazz)) {
				this.isImport = true;
				return;
			}
		}
