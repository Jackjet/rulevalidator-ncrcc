 ...
 * @version 1.0.0.0
 */
@RuleDefinition(executeLayer = ExecuteLayer.BUSICOMP, executePeriod = ExecutePeriod.COMPILE, catalog = CatalogEnum.JAVACODE, subCatalog = SubCatalogEnum.JC_CODECRITERION, description = "提交时,前规则中是否有启用审批流的检查规则", relatedIssueId = "300", coder = "lijbe", solution = "通过关键字符CommitBP取bs包下的*CommitBP.java文件;再通过关键字符nc.bs.pubapp.pub.rule.WorkflowCheckRule.判断BP是否引用了规则."
        + "注意添加前规则的方法名为addBeforeRule.", memo = "该条规则有审批流时检查")
public class TestCase00300 extends AbstractJavaQueryRuleDefinition {
