...
/**
 * 主数量的编缉后事件中必须处理主辅计量换算
 * 
 * @author qiaoyanga
 * @since V1.0
 * @version 1.0.0.0
 */
@RuleDefinition(executePeriod = ExecutePeriod.DEPLOY, catalog = CatalogEnum.JAVACODE,
		subCatalog = SubCatalogEnum.JC_CODECRITERION, description = "主数量的编缉后事件中必须处理主辅计量换算 ", relatedIssueId = "231",
		coder = "qiaoyanga", solution = "配置功能节点号参数，将通过节点号找到对应的配置文件，通过配置文件找到表头表体编辑后事件类，"
				+ "判断类中是有以'NumberHandler'结尾的换算率事件处理类。再分析具体换算率类中后事件是否有引用‘nc.ui.mmf.busi.measure.CalculatorUtil’进行主辅计算的类")
public class TestCase00231 extends AbstractXmlRuleDefinition {
	@Override
				...
				try {
					...
					if (visitor.getEventname() == null) {
						result.addResultElement(xmlResource.getResourcePath(), className
								+ "中未找到以‘NumberHandler’结尾的主数量编辑事件\n");
						continue;
					}
					...
					if (javaList == null || javaList.size() <= 0) {
						result.addResultElement(javaClassResource.getResourcePath(), "中未找到以‘NumberHandler’结尾的主数量编辑事件\n");
						continue;
					}
					...
					if (!eventVisitor.isImport()) {
						result.addResultElement(javaClass.getResourcePath(), "主数量的编缉后事件中未处理主辅计量换算\n");
						continue;
					}
	...
	private static class Visitor extends VoidVisitorAdapter<Void> {
		...
		public void visit(ImportDeclaration n, Void arg) {
			...
			if (importClazz.contains("NumberHandler")) {
