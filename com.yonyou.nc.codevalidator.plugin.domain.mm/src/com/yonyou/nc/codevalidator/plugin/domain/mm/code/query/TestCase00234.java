...
@RuleDefinition(executePeriod = ExecutePeriod.DEPLOY, catalog = CatalogEnum.JAVACODE,
		subCatalog = SubCatalogEnum.JC_CODEDESIGN, description = "要进行冗余字段的处理，以便查询时应该自动拼接SQL条件 ",
		relatedIssueId = "234", coder = "qiaoyanga",
		solution = "配置节点号参数，将通过节点号找到对应的配置文件，通过配置文件找到查询初试化类，判断类中是有'addRedundancyInfo'方法")
public class TestCase00234 extends AbstractXmlRuleDefinition {
	@Override
	...
	protected IRuleExecuteResult processScriptRules(IRuleExecuteContext ruleExecContext, List<XmlResource> resources)
		...
		for (XmlResource xmlResource : resources) {
			...
			try {
				...
				if (!isImport) {
					result.addResultElement(javaClassResource.getJavaCodeClassName(), "查询初试化类中需要添加冗余字段过滤 \n");
					continue;
				}
	...
	private static class QryCondDLGInitializer extends VoidVisitorAdapter<Void> {
		...
		public void visit(MethodDeclaration n, Void v) {
			...
			for (Statement stmt : stmts) {
				if (stmt.toString().contains("addRedundancyInfo")) {
					this.flag = true;
					break;
				}
			}
