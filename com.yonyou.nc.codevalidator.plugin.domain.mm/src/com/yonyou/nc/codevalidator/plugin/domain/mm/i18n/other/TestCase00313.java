 ...
 * 
 * @since 6.0
 * @version 2013-9-1 上午9:54:13
 * @author zhongcha
 */

@RuleDefinition(executeLayer = ExecuteLayer.GLOBAL, catalog = CatalogEnum.LANG, subCatalog = SubCatalogEnum.LANG_COMMON, description = "检查单据类型名称多语是否做处理", specialParamDefine = {
    CommonRuleParams.NODECODEPARAM
}, solution = "添加该单据类型名称多语，即bd_billtype中billtypename字段不为空，且 istransaction='N' ", coder = "zhongcha", relatedIssueId = "313", memo = "多个参数请用,隔开（节点编码即NODECODE）")
        ...
        if (MMValueCheck.isNotEmpty(errorMapList)) {
            for (Map.Entry<String, List<String>> entry : errorMapList.entrySet()) {
                result.addResultElement(entry.getKey() + MmMDi18nConstants.ERROR_DATABASE_NULL,
                        "节点编码为[" + entry.getKey() + "]的单据类型编码为" + entry.getValue() + "的单据类型未做多语。\n");
            }
        }
        if (MMValueCheck.isNotEmpty(errorList)) {
            result.addResultElement(MmMDi18nConstants.ERROR_DATABASE_NULL, "单据类型编码为" + errorList
                    + "的单据类型未做多语(注：未找到对应的节点编码)。\n");
