...
/**
 * 单据复制时必须清除掉主表的主键、单据号、业务日期、单据状态、审批信息、审计信息、ts；
 * 
 * @since 6.0
 * @version 2013-11-5 上午9:07:20
 * @author zhongcha
 */
@RuleDefinition(executePeriod = ExecutePeriod.CHECKOUT, catalog = CatalogEnum.JAVACODE, subCatalog = SubCatalogEnum.JC_CODECRITERION, description = "单据复制时必须清除掉主表的主键、单据号、业务日期、单据状态、审批信息、审计信息、ts；  ", relatedIssueId = "214", coder = "zhongcha", solution = "请在该复制按钮处理类中添加指定字段的置空操作。")
public class TestCase00214 extends AbstractMetadataResourceRuleDefinition {
    ...
    protected IRuleExecuteResult processResources(List<MetadataResource> resources, IRuleExecuteContext ruleExecContext)
        ...
        for (MetadataResource metadataResource : resources) {
            ...
            if (MMValueCheck.isNotEmpty(mainEntity)) {
                ...
                if (MMValueCheck.isNotEmpty(aggvoName)) {
                    Map<String, IAttribute> ibdoMap =
                            mainEntity.getBusiInterfaceAttributes(MDRuleConstants.IBDOBJECT_INTERFACE_NAME);
                    if (MMValueCheck.isNotEmpty(ibdoMap)) {
                        // 获得主表的主键
                        aggvo2Attributes.put(aggvoName, ibdoMap.get("id"));
                    }
                    Map<String, IAttribute> auditMap =
                            mainEntity.getBusiInterfaceAttributes(MDRuleConstants.AUDITINFO_NAME);
                    if (MMValueCheck.isNotEmpty(auditMap)) {
                        aggvo2Attributes.put(aggvoName, auditMap.get("创建人"));
                        aggvo2Attributes.put(aggvoName, auditMap.get("创建时间"));
                        aggvo2Attributes.put(aggvoName, auditMap.get("最后修改人"));
                        aggvo2Attributes.put(aggvoName, auditMap.get("最后修改时间"));
                    }
                    ...
                    if (MMValueCheck.isNotEmpty(flowbizMap)) {
                        aggvo2Attributes.put(aggvoName, flowbizMap.get("制单人"));
                        aggvo2Attributes.put(aggvoName, flowbizMap.get("审批人"));
                        aggvo2Attributes.put(aggvoName, flowbizMap.get("审批时间"));
                        aggvo2Attributes.put(aggvoName, flowbizMap.get("审批状态"));
                        if (MMValueCheck.isNotEmpty(flowbizMap.get("审批状态"))) {
                            // 记录单据状态字段的名称（单据状态和其他字段不同，它不是做置空操作，该TestCase中只做到判断存在对单据状态进行处理的代码，即认为处理正确。）
                            fbillstatus = flowbizMap.get("审批状态").getFieldName();
                        }
                    }
                }
            }
            ...
            try {
                ...
                if (MMValueCheck.isNotEmpty(scanRule.getFieldName2Result())) {
                    for (String key : scanRule.getFieldName2Result().keySet()) {
                        if (!scanRule.getFieldName2Result().get(key).equals(Integer.valueOf(2))) {
                            if (key.equals(fbillstatus)
                                    && scanRule.getFieldName2Result().get(key).equals(Integer.valueOf(1))) {
                                continue;
                            }
        ...
        if (MMValueCheck.isNotEmpty(errorMapList)) {
            for (String key : errorMapList.keySet()) {
                errorContxt
                        .append("在复制按钮处理类[ " + key + " ]中字段名" + errorMapList.get(key) + "的字段没做置空处理（其中单据状态应设为自由态）。\n");
            }
        }
    ...
    private static class ScanRule extends AbstractJavaRule {
        ...
        public Object visit(ASTName node, Object data) {
                        ...
                        if (MMValueCheck.isNotEmpty(node.getFirstParentOfType(ASTPrimaryPrefix.class))
                            ...
                            if (priExpression.hasDescendantOfType(ASTArguments.class)) {
                                if (priExpression.findDescendantsOfType(ASTArguments.class).get(0)
                                    ...
                                    this.fieldName2Result.put(this.fieldMethod2Name.get(fieldMethod),
                                            Integer.valueOf(2));
                                    break;
                                }
                            }
            ...
            if (MMValueCheck.isNotEmpty(this.fieldUpCase2DownCase) && MMValueCheck.isNotEmpty(node.getImage())) {
                ...
                if (image.endsWith(".setAttributeValue")) {
                                ...
                                for (String fieldUpCase : this.fieldUpCase2DownCase.keySet()) {
                                    if (itemname.endsWith(fieldUpCase)) {
                                        this.fieldName2Result.put(this.fieldUpCase2DownCase.get(fieldUpCase),
                                                Integer.valueOf(1));
                                        // 判读第二个参数是否为null
                                        if (expressions.get(1).hasDescendantOfType(ASTNullLiteral.class)) {
                                            this.fieldName2Result.put(this.fieldUpCase2DownCase.get(fieldUpCase),
                                                    Integer.valueOf(2));
                                        }
                                        break;
                ...
                else if (image.endsWith(".setPrimaryKey")) {
                    this.fieldName2Result.put(this.fieldUpCase2DownCase.get(".setPrimaryKey"), Integer.valueOf(1));
                    ...
                    if (MMValueCheck.isNotEmpty(node.getFirstParentOfType(ASTPrimaryPrefix.class))
                        ...
                        if (priExpression.hasDescendantOfType(ASTArguments.class)) {
                            if (priExpression.findDescendantsOfType(ASTArguments.class).get(0)
                                    .hasDescendantOfType(ASTNullLiteral.class)) {
                                this.fieldName2Result.put(this.fieldUpCase2DownCase.get(".setPrimaryKey"),
                                        Integer.valueOf(2));
                            }
                        }
                    }
                ...
                for (String key : this.aggvo2fieldNames.keySet()) {
                    if (key.equals(image)) {
                        for (String fieldName : this.aggvo2fieldNames.get(key)) {
                            if (MMValueCheck.isEmpty(this.fieldMethod2Name)) {
                                // 主键特殊处理(因为this.aggvo2fieldNames.get(key)第一个是主键)
                                this.fieldUpCase2DownCase.put(".setPrimaryKey", fieldName);
                            }
                            else {
