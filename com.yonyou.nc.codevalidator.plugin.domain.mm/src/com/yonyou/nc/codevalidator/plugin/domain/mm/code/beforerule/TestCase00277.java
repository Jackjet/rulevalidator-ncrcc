...
/**
 * 更新BP的前规则：单据号的更新规则(nc.bs.pubapp.pub.rule.UpdateBillCodeRule)
 * 
 * @since 6.0
 * @version 2013-11-4 上午10:53:17
 * @author zhongcha
 */
@RuleDefinition(executePeriod = ExecutePeriod.CHECKOUT, catalog = CatalogEnum.JAVACODE, subCatalog = SubCatalogEnum.JC_CODECRITERION, description = "更新BP的前规则：单据号的更新规则(nc.bs.pubapp.pub.rule.UpdateBillCodeRule)。", relatedIssueId = "277", coder = "zhongcha", solution = "在前规则中添加单据号的更新规则(nc.bs.pubapp.pub.rule.UpdateBillCodeRule)。")
public class TestCase00277 extends AbstractJavaQueryRuleDefinition {
        ...
        try {
            ...
            if (MMValueCheck.isNotEmpty(notRuleClz)) {
                errorContxt.append("该组件中名为" + notRuleClz
                        + "的BP类的addBeforeRule的方法中没有执行单据号的更新规则(nc.bs.pubapp.pub.rule.UpdateBillCodeRule)。\n");
            }
    ...
    private static class ScanUpdateBPRule extends AbstractJavaRule {
        ...
        /**
         * 扫描方法，当当前方法为addBeforeRule时，判断是否存在判断单据状态的规则
         */
        @Override
                    ...
                    for (ASTBlockStatement blockStatemt : blockStatemtList) {
                        if (MMValueCheck
                            ...
                            if (MMValueCheck.isNotEmpty(ruleType) && MMValueCheck.isNotEmpty(ruleType.getImage())) {
                                ...
                                if ("nc.bs.pubapp.pub.rule.UpdateBillCodeRule".equals(image)) {
