...
@RuleDefinition(
		...
		description = "物料的编辑后事件需要根据主辅数量和单位计算主辅换算率",
		relatedIssueId = "225",
		coder = "lijbe",
...
public class TestCase00225 extends AbstractXmlRuleDefinition {
	...
	private void checkConcreteHandler(JavaClassResource javaClassResource) {
		...
		try {
			...
			if (!visitor.isMutilRow || !visitor.isCalculate) {
				noteBuilder
						.append(String
								.format("物料编辑事件处理类【%s】编辑后没有根据主辅数量和单位设置换算率,即没有调用【nc.ui.mmf.busi.measure.handler.MaterialvidHandler.afterEdit方法】和【nc.ui.mmf.busi.measure.CalculatorUtil.calculate方法】\n",
										javaClassResource.getJavaCodeClassName()));
			}
		} catch (ParseException e) {
			Logger.error(e.getMessage(), e);
