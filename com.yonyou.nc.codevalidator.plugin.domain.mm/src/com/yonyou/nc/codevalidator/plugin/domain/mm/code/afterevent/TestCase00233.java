...
@RuleDefinition(
		...
		description = "换算率的编缉后事件中必须处理主辅计量换算 ",
		relatedIssueId = "233",
		coder = "qiaoyanga",
		solution = "配置功能节点号参数，将通过节点号找到对应的配置文件，通过配置文件找到表头表体编辑后事件类，"
				+ "判断类中是有以'ChangeRateHandler'结尾的换算率事件处理类。再分析具体换算率类中后事件是否有引用‘nc.ui.mmf.busi.measure.CalculatorUtil’进行主辅计算的类")
public class TestCase00233 extends AbstractXmlRuleDefinition {
	@Override
			...
			for (String className : classNames) {
				try {
					...
					if (!eventVisitor.isImport()) {
						result.addResultElement(javaClass.getResourcePath(), "换算率的编缉后事件中未处理主辅计量换算\n");
						continue;
					}
