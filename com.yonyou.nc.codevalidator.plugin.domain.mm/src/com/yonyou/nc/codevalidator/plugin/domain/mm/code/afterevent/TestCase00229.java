...
/**
 * 业务单位的编缉后事件中必须处理主辅换算 1,先按照关键字"*unitHandler.java",
...
@RuleDefinition(executeLayer = ExecuteLayer.BUSICOMP, executePeriod = ExecutePeriod.DEPLOY,
		...
		description = "业务单位的编缉后事件中必须处理主辅计量的换算 ", relatedIssueId = "229", coder = "lijbe",
		solution = "1.通过编辑后事件类型找到事件监听处理类;2.根据关键字符*UnitHandler或者*unitHandler【单位编辑事件命名以其结尾】找到编辑事件处理类;"
				+ "2.分析afterEdit中是否调用了【nc.ui.mmf.busi.measure.CalculatorUtil.calculate方法】")
public class TestCase00229 extends AbstractXmlRuleDefinition {
	...
	private void checkConcreteHandler(JavaClassResource javaClassResource) {
		...
		try {
			...
			visitor.visit(compilationUnit, null);
			if (!visitor.isCalculate) {
				noteBuilder
						.append(String
								.format("业务单位编辑事件处理类【%s】编辑后事件中没有处理主辅计量换算,即没有调用【nc.ui.mmf.busi.measure.CalculatorUtil.calculate方法】进行处理.\n",
										javaClassResource.getJavaCodeClassName()));
			}
		} catch (ParseException e) {
			Logger.error(e.getMessage(), e);
