 ...
 *
 */
@RuleDefinition(executePeriod = ExecutePeriod.DEPLOY,catalog = CatalogEnum.UIFACTORY, subCatalog = SubCatalogEnum.UF_CONFIGFILE, description = "支持单据号可编辑的配置项billCodeMediator（可选）。", coder = "wangfra", relatedIssueId = "211")
	...
	protected IRuleExecuteResult processScriptRules(
		...
		for(XmlResource xmlResource : resources){
			...
			if (listenerList == null || listenerList.size() == 0) {
				 result.addResultElement(xmlResource.getResourcePath(),"xml文件未支持billCodeMediator--nc.ui.pubapp.bill.BillCodeMediator \n");
			}
		}
