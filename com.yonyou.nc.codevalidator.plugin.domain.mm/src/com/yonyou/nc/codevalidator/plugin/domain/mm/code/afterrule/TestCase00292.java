 ...
 * @version 1.0.0.0
 */
@RuleDefinition(executePeriod = ExecutePeriod.CHECKOUT, catalog = CatalogEnum.JAVACODE, subCatalog = SubCatalogEnum.JC_CODECRITERION, description = "删除时，后规则中是否有单据号回退规则【单据可选】", relatedIssueId = "292", coder = "lijbe", solution = "通过关键字符*DeleteBP取bs包下的*DeleteBP.java文件;再判断BP是否引用了关键字符nc.bs.pubapp.pub.rule.ReturnBillCodeRule;注意添加后规则的方法名为addAfterRule.")
public class TestCase00292 extends AbstractJavaQueryRuleDefinition {
        ...
        try {
            ...
            VoidVisitorAdapter<Void> visitor = new VoidVisitorAdapter<Void>() {
                ...
                public void visit(MethodDeclaration n, Void v) {
                    if ("addAfterRule".equals(n.getName())) {
                        ...
                        for (Statement stmt : stmts) {
                            if (stmt.toString().contains("ReturnBillCodeRule")) {
                ...
                private void appendMsg(String methodName) {
                    noteBuilder.append(String.format("方法【%s】没有添加单据号回退规则【nc.bs.pubapp.pub.rule.ReturnBillCodeRule】！\n",
