...
/**
 * 主组织改变时单据上的参照根据主组织进行过滤
 * 
 * @since 6.0
 * @version 2013-11-4 上午10:53:17
 * @author zhongcha
 */
@RuleDefinition(catalog = CatalogEnum.JAVACODE, subCatalog = SubCatalogEnum.JC_CODECRITERION, description = "主组织改变时单据上的参照根据主组织进行过滤 。", relatedIssueId = "218", coder = "zhongcha", solution = "在主组织改变事件处理类中添加参照根据主组织进行过滤代码（BillPanelUtils.setOrgForAllRef） 。")
public class TestCase00218 extends AbstractJavaQueryRuleDefinition {
    ...
    protected IRuleExecuteResult processJavaRules(IRuleExecuteContext ruleExecContext, List<JavaClassResource> resources)
        ...
        try {
            ...
            if (!notMethodClz.isEmpty()) {
                errorContxt.append("该组件中名为" + notMethodClz + "的组织改变事件处理类中没有设置参照根据主组织进行过滤。\n");
            }
    ...
    private static class ScanRule extends AbstractJavaRule {
        ...
        public Object visit(ASTName node, Object data) {
            if (this.isCorrectEvent) {
                ...
                if (image.equals("BillPanelUtils.setOrgForAllRef")) {
