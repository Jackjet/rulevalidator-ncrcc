 ...
 * @version 1.0.0.0
 */
@RuleDefinition(executePeriod = ExecutePeriod.DEPLOY, catalog = CatalogEnum.JAVACODE, subCatalog = SubCatalogEnum.JC_CODEDESIGN, description = "增加自由辅助属性的处理器", relatedIssueId = "242", coder = "qiaoyanga", solution = "配置节点号参数，将通过节点号找到对应的配置文件，通过配置文件找到查询初试化类，判断类中是否有'addQueryCondVODealer'方法")
public class TestCase00242 extends AbstractXmlRuleDefinition {
    @Override
    ...
    protected IRuleExecuteResult processScriptRules(IRuleExecuteContext ruleExecContext, List<XmlResource> resources)
        ...
        for (XmlResource xmlResource : resources) {
            ...
            try {
                ...
                if (!isImport) {
                    result.addResultElement(javaClassResource.getJavaCodeClassName(), "查询初始化类中未添加自由辅助属性的处理器 \n");
                    continue;
                }
            }
    ...
    private static class QryCondDLGInitializer extends VoidVisitorAdapter<Void> {
        ...
        public void visit(MethodDeclaration n, Void v) {
            ...
            for (Statement stmt : stmts) {
                if (stmt.toString().contains("addQueryCondVODealer")) {
                    this.flag = true;
                }
            }
        }
