 ...
 * @version 1.0.0.0
 */
@RuleDefinition(executePeriod = ExecutePeriod.DEPLOY, catalog = CatalogEnum.UIFACTORY, subCatalog = SubCatalogEnum.UF_LAYOUT, description = "检查卡片视图billFormEditor是否支持物料自由辅助属性[可选]", relatedIssueId = "853", coder = "lijbe", solution = "检查卡片视图billFormEditor应该支持物料自由辅助属性，并且其实现类为nc.ui.pubapp.uif2app.view.material.assistant.MarAsstPreparator,而且"
        + "应该将其注入到billFormEditor中")
public class TestCase00853 extends AbstractXmlRuleDefinition {
    ...
    protected IRuleExecuteResult processScriptRules(IRuleExecuteContext ruleExecContext, List<XmlResource> resources) {
        ...
        if (funNodes.size() > 0) {
            result.addResultElement(ruleExecContext.getBusinessComponent().getDisplayBusiCompName(), String.format(
                    "功能编码为 %s 的节点配置文件中卡片视图没有配置物料辅助属性类：%s \n", funNodes, MmUIFactoryConstants.MAR_ASST_PREPARATOR_CLASS));
        }
        if (funNodesTwo.size() > 0) {
            result.addResultElement(ruleExecContext.getBusinessComponent().getDisplayBusiCompName(),
                    String.format("功能编码为 %s 的节点配置文件卡片视图没有引用物料辅助属性 %s  \n", funNodesTwo, "marAsstPreparator"));
        }
