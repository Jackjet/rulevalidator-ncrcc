...
/**
 * 如果客户作为查询条件，则客户ID、客户编码、客户名称需要进行参照过滤
 * 
 * @author qiaoyanga
 * @since V1.0
 * @version 1.0.0.0
...
@RuleDefinition(
		...
		description = "如果客户作为查询条件，则客户ID、客户编码、客户名称需要进行参照过滤 ",
		relatedIssueId = "237",
		coder = "qiaoyanga",
		solution = "配置节点号参数，将通过节点号找到对应的配置文件，通过配置文件找到查询初试化类，判断类中是否有客户参照过滤类‘nc.ui.mmf.busi.query.reffilter.QCustomerFilter’")
public class TestCase00237 extends AbstractXmlRuleDefinition {
	@Override
	...
	protected IRuleExecuteResult processScriptRules(IRuleExecuteContext ruleExecContext, List<XmlResource> resources)
		...
		for (XmlResource xmlResource : resources) {
			...
			try {
				...
				if (!isImport) {
					result.addResultElement(javaClassResource.getJavaCodeClassName(), "查询初试化类中客户需要进行参照过滤 \n");
					continue;
				}
	...
	private static class QryCondDLGInitializer extends VoidVisitorAdapter<Void> {
		...
		public void visit(ImportDeclaration n, Void arg) {
			...
			if ("nc.ui.mmf.busi.query.reffilter.QCustomerFilter".equals(importClazz)) {
				this.isImport = true;
				return;
			}
		}
