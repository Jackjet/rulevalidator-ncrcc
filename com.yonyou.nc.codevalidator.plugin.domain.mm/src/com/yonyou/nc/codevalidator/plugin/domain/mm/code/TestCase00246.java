...
/**
 * 列表界面继承ShowUpableBillListView类
 * @author gaojf
 *
 */
@RuleDefinition(executeLayer = ExecuteLayer.BUSICOMP, executePeriod = ExecutePeriod.DEPLOY,catalog = CatalogEnum.JAVACODE,description = "列表界面继承ShowUpableBillListView类", relatedIssueId = "246", subCatalog = SubCatalogEnum.JC_CODECRITERION, coder = "gaojf")
			...
			if(!isExtends){
				result.addResultElement(ruleExecContext.getBusinessComponent()
						.getDisplayBusiCompName(),String.format(
								"功能编码为：%s 的配置文件中，列表类没有继承ShowUpableBillListView. \n",
