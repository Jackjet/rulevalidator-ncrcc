...
/**
 * 列表视图listView，如果没有多个单据模板，不要配置nodeKeies选项
 * 
 * @author lijbe
 * @since V1.0
 * @version 1.0.0.0
 */
@RuleDefinition(executePeriod = ExecutePeriod.DEPLOY, catalog = CatalogEnum.UIFACTORY, subCatalog = SubCatalogEnum.UF_LAYOUT, description = "列表视图listView,如果没有多个单据模板,不要配置nodeKeies选项", relatedIssueId = "172", coder = "lijbe", solution = "列表视图listView,如果没有多个单据模板,不要配置nodeKeies选项.【列表视图bean的id必须为listView】")
public class TestCase00172 extends AbstractXmlRuleDefinition {
    ...
    protected IRuleExecuteResult processScriptRules(IRuleExecuteContext ruleExecContext, List<XmlResource> resources)
        ...
        for (XmlResource xmlResource : resources) {
            Element billFormEditorElement = xmlResource.getElementById("listView");
        ...
        if (wrongNameNodes.size() == resources.size()) {
            result.addResultElement(ruleExecContext.getBusinessComponent().getDisplayBusiCompName(),
                    String.format("功能编码为：%s 的节点配置文件中没有配置列表视图,或者列表视图的名称不为：%s \n", wrongNameNodes, "listView"));
        ...
        if (unlawfulNodes.size() > 0) {
            result.addResultElement(ruleExecContext.getBusinessComponent().getDisplayBusiCompName(), String.format(
                    "功能编码为：%s 的节点只有一个单据模板,模板容器templateContainer不用配置nodeKeies,并且列表视图listView不用注入属性:nodekey. \n",
                    wrongNameNodes));
        }
        return result;
    }
    ...
    private MapList<String, String> getBillTemplateCount(String[] nodeCodes, IRuleExecuteContext ruleExecContext)
        ...
        whereValue += ") and dr = 0 and pk_corp='@@@@' ";
        sql += whereValue;
    ...
    private boolean checkOneTemplateWithNodeKeies(int templateNum, XmlResource xmlResource) {
        ...
        Element listViewElement = xmlResource.getElementById("listView");

        Element nodeKeyElement = xmlResource.getChildPropertyElement(listViewElement, "nodekey");
