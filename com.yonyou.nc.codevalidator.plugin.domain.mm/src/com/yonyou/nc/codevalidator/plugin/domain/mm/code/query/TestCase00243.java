...
/**
 * 如果单据支持自定义项和自由辅助属性，则需要进行组织参照过滤
 * 
 * @author qiaoyanga
 * @since V1.0
 * @version 1.0.0.0
 */
@RuleDefinition(executePeriod = ExecutePeriod.DEPLOY, catalog = CatalogEnum.JAVACODE, subCatalog = SubCatalogEnum.JC_CODEDESIGN, description = "如果单据支持自定义项和自由辅助属性，则需要进行组织参照过滤 ", relatedIssueId = "243", coder = "qiaoyanga", solution = "配置节点号参数，将通过节点号找到对应的配置文件，通过配置文件找到查询初试化类，判断类中是否有引用‘自定义项与自由项,启用成参照,用组织过滤的类’：’nc.ui.mmf.busi.query.reffilter.QMarAndDefFilter‘")
public class TestCase00243 extends AbstractXmlRuleDefinition {
    @Override
    ...
    private static class QryCondDLGInitializer extends VoidVisitorAdapter<Void> {
        ...
        public void visit(ImportDeclaration n, Void arg) {
            ...
            if ("nc.ui.mmf.busi.query.reffilter.QMarAndDefFilter".equals(importClazz)) {
                this.isImport = true;
                return;
            }
        }
