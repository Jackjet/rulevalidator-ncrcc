...
/**
 * 检查单据是否支持了表头表尾字段编辑前事件
 * 
 * @author lijbe
 * @since V1.0
 * @version 1.0.0.0
 */
@RuleDefinition(executePeriod = ExecutePeriod.DEPLOY, catalog = CatalogEnum.UIFACTORY, subCatalog = SubCatalogEnum.UF_BTNOREVENT, description = "检查单据是否支持了表头表尾字段编辑前事件", solution = "检查单据是否支持了表头表尾字段编辑前事件[nc.ui.pubapp.uif2app.event.card.CardHeadTailBeforeEditEvent]", coder = "lijbe", relatedIssueId = "183")
public class TestCase00183 extends AbstractXmlRuleDefinition {
    ...
    protected IRuleExecuteResult processScriptRules(IRuleExecuteContext ruleExecContext, List<XmlResource> resources)
        ...
        for (XmlResource xmlResource : resources) {
            ...
            for (Element element : eventElement) {
                ...
                if (null != e && MmUIFactoryConstants.HEAD_TAIL_BEFORE_EDIT_EVENT.equals(e.getAttribute("value"))) {
                    flag = true;
