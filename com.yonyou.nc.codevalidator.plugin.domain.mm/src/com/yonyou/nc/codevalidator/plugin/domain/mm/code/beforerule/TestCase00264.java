...
/**
 * 前规则：单据号的创建规则 1.通过关键字符*InsertBP.java取bs包下的*InsertBP.java文件;2.再通过关键字符
 * nc.bs.pubapp.pub.rule.CreateBillCodeRule.判断BP是否引用了
 * 
 * @author lijbe
 * @since V1.0
 * @version 1.0.0.0
 */
@RuleDefinition(executeLayer = ExecuteLayer.BUSICOMP, executePeriod = ExecutePeriod.COMPILE, catalog = CatalogEnum.JAVACODE, subCatalog = SubCatalogEnum.JC_CODECRITERION, description = "前规则中是否有单据号的创建规则", relatedIssueId = "264", coder = "lijbe", solution = "通过关键字符*InsertBP取bs包下的*InsertBP.java文件;再判断BP的addBeforeRule方法是否添加了nc.bs.pubapp.pub.rule.CreateBillCodeRule")
public class TestCase00264 extends AbstractJavaQueryRuleDefinition {
        ...
        try {
            ...
            VoidVisitorAdapter<Void> visitor = new VoidVisitorAdapter<Void>() {
                ...
                public void visit(MethodDeclaration n, Void v) {
                    if (methodNames.contains(n.getName())) {
                        ...
                        for (Statement stmt : stmts) {
                            if (stmt.toString().contains("CreateBillCodeRule")) {
                ...
                private void appendMsg(String methodName) {
                    noteBuilder.append(String.format("方法【%s】没有添加单据号的创建规则【nc.bs.pubapp.pub.rule.CreateBillCodeRule】\n!",
