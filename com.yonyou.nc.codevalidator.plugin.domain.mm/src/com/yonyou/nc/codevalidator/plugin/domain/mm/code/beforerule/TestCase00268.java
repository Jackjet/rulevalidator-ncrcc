...
/**
 * 单据字段长度的校验规则
 * 1.通过关键字符*InsertBP.java取bs包下的*InsertBP.java文件;2.再通过关键字符nc.bs.pubapp
 * .pub.rule.FieldLengthCheckRule.判断BP是否引用了
 * 
 * @author lijbe
 * @since V1.0
 * @version 1.0.0.0
 */
@RuleDefinition(executeLayer = ExecuteLayer.BUSICOMP, executePeriod = ExecutePeriod.COMPILE, catalog = CatalogEnum.JAVACODE, subCatalog = SubCatalogEnum.JC_CODECRITERION, description = "前规则中是否添加了字段长度检查规则", relatedIssueId = "268", coder = "lijbe", solution = "通过关键字符*InsertBP取bs包下的*InsertBP.java文件;再判断BP是否引用了关键字符nc.bs.pubapp.pub.rule.FieldLengthCheckRule;注意添加前规则的方法名为addBeforeRule.")
public class TestCase00268 extends AbstractJavaQueryRuleDefinition {
            ...
            VoidVisitorAdapter<Void> visitor = new VoidVisitorAdapter<Void>() {
                ...
                public void visit(MethodDeclaration n, Void v) {
                    if (methodNames.contains(n.getName())) {
                        ...
                        for (Statement stmt : stmts) {
                            if (stmt.toString().contains("FieldLengthCheckRule")) {
                ...
                private void appendMsg(String methodName) {
                    noteBuilder.append(String.format(
                            "方法【%s】没有添加单据字段长度检查规则【nc.bs.pubapp.pub.rule.FieldLengthCheckRule】\n!",
