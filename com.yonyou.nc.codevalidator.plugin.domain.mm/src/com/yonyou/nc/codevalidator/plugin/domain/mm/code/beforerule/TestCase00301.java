...
/**
 * 取消审批的BP需要判断单据状态的规则(默认取消审批BP命名格式为：后缀是UnApproveBP；若该BP不是如此命名，则需传入参数。)
 * 
 * @since 6.0
...
@RuleDefinition(executePeriod = ExecutePeriod.CHECKOUT, catalog = CatalogEnum.JAVACODE, subCatalog = SubCatalogEnum.JC_CODECRITERION, specialParamDefine = {
    "检查规则"
}, description = "取消审批的BP需要判断单据状态的检查规则(若没有参数时，检查默认场景：更新BP命名格式为后缀是UnApproveBP，规则类为UnapproveStatusCheckRule。有参数，若该组件下只有一个节点，参数直接为规则（如：DomUpdateStatusCheckRule）,若存在多个节点，则参数形如dmo@CheckDmoStatusRule#BillUpdateStatusCheckRule（代表：节点编码为dmo下的BP类检测CheckDmoStatusRule和BillUpdateStatusCheckRule），指定节点下只检查指定的规则，组件中若还存在其他节点的BP，执行默认的或者参数中不带节点的其他规则的检查。)  ", relatedIssueId = "301", coder = "zhongcha", solution = "在前规则中添加判断单据状态的规则。", memo = "参数可空。存在多个参数请用，隔开。若该组件下只有一个节点，参数内容为需检查的规则（如：DomUpdateStatusCheckRule）,若存在多个节点，则参数形如dmo@CheckDmoStatusRule#BillUpdateStatusCheckRule（代表：节点编码为dmo下的BP类检测CheckDmoStatusRule和BillUpdateStatusCheckRule），指定节点下只检查指定的规则，组件中若还存在其他节点的BP，执行默认的或者参数中不带节点的其他规则的检查。)")
public class TestCase00301 extends AbstractJavaQueryRuleDefinition {
        ...
        try {
            ...
            if (MMValueCheck.isEmpty(package2BPClzs)) {
                result.addResultElement(ruleExecContext.getBusinessComponent().getDisplayBusiCompName(),
                        "该组件中不存在类名后缀为UnApproveBP的取消审批BP。\n");
                return result;
            }
    ...
    private static class ScanUnApproveBPRule extends AbstractJavaRule {
        ...
        /**
         * 当当前扫描的方法为addBeforeRule时，判断是否存在判断单据状态的规则
         */
        @Override
