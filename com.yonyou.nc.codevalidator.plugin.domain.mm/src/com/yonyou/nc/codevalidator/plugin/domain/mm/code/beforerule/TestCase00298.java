...
/**
 * 取消提交时单据状态的检查规则
 * 1.通过关键字符*UnCommitBP.java取bs包下的*UnCommitBP.java文件;2.再通过关键字符nc.bs.pubapp
 * .pub.rule.UncommitStatusCheckRule或者注入的rule.判断BP是否引用了
 * 
 * @author lijbe
 * @since V1.0
 * @version 1.0.0.0
 */
@RuleDefinition(executePeriod = ExecutePeriod.COMPILE, catalog = CatalogEnum.JAVACODE, subCatalog = SubCatalogEnum.JC_CODECRITERION, description = "取消提交时,前规则中是否进行有单据状态检查规则", relatedIssueId = "298", coder = "lijbe", specialParamDefine = "检查规则", solution = "通过关键字符UnCommitBP取bs包下的*UnCommitBP.java文件;再通过关键字符nc.bs.pubapp.pub.rule.UncommitStatusCheckRule或者注入的rule【一般都使用默认的】.判断BP是否引用了规则,参数规则为节点编码@规则名,比如离散生产订单dmo,"
        + "就是dmo@CheckDmoStatusRule,如果业务组件下有多个节点，那么使用#将其分隔开;注意添加前规则的方法名为addBeforeRule.【如果没有录入参数，默认检查是否存在规则nc.bs.pubapp.pub.rule.UncommitStatusCheckRule】")
public class TestCase00298 extends AbstractJavaQueryRuleDefinition {
    ...
    protected IRuleExecuteResult processJavaRules(IRuleExecuteContext ruleExecContext, List<JavaClassResource> resources)
        ...
        // .getParameterArray(TestCase00298.PARAM_NAME))) {
        // result.addResultElement(ruleExecContext.getBusinessComponent()
        // .getBusinessComp(), "请在参数中输入取消提交时单据状态检查规则. \n");
        // return result;
        // }
        String[] params = ruleExecContext.getParameterArray(TestCase00298.PARAM_NAME);
        ...
        for (final JavaClassResource javaClassResource : resources) {
            ...
            if (className.endsWith("UnCommitBP")) {
        ...
        if (MMMapUtil.isEmpty(javaClazzResMapList)) {
            result.addResultElement(ruleExecContext.getBusinessComponent().getBusinessComp(),
                    "没有找到以UnCommitBP结尾的java文件，请检查插入BP命名是否以UnCommitBP结尾. \n");
            return result;
        }
        ...
        try {
            ...
            if (MMValueCheck.isNotEmpty(methodNames)) {
                if (MMValueCheck.isEmpty(ruleName)) {
                    tmpRuleName.append("UncommitStatusCheckRule");
                }
                else {
            ...
            VoidVisitorAdapter<Void> visitor = new VoidVisitorAdapter<Void>() {
                ...
                private void appendMsg(String methodName) {
                    noteBuilder.append(String.format("方法【%s】没有添加取消审批时的单据状态检查规则【" + tmpRuleName.toString() + "】\n!",
