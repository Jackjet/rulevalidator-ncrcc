...
/**
 * 如果项目作为查询条件，则项目ID、项目编码、项目名称需要进行参照过滤
 * 
 * @author qiaoyanga
 * @since V1.0
 * @version 1.0.0.0
...
@RuleDefinition(
		...
		description = "如果项目作为查询条件，则项目ID、项目编码、项目名称需要进行参照过滤 ",
		relatedIssueId = "240",
		coder = "qiaoyanga",
		solution = "配置节点号参数，将通过节点号找到对应的配置文件，通过配置文件找到查询初试化类，判断类中是否有项目参照过滤类‘nc.ui.mmf.busi.query.reffilter.QProjectFilter’")
public class TestCase00240 extends AbstractXmlRuleDefinition {
	...
	protected IRuleExecuteResult processScriptRules(IRuleExecuteContext ruleExecContext, List<XmlResource> resources)
		...
		for (XmlResource xmlResource : resources) {
			...
			try {
				...
				if (!isImport) {
					result.addResultElement(javaClassResource.getJavaCodeClassName(), "查询初试化类中项目需要进行参照过滤 \n");
					continue;
				}
	...
	private static class QryCondDLGInitializer extends VoidVisitorAdapter<Void> {
		...
		public void visit(ImportDeclaration n, Void arg) {
			...
			if ("nc.ui.mmf.busi.query.reffilter.QProjectFilter".equals(importClazz)) {
				this.isImport = true;
				return;
			}
		}
