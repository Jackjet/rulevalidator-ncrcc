 ...
 * 
 * @since 6.0
 * @version 2013-9-5 下午8:10:42
 * @author zhongcha
 */
@RuleDefinition(executePeriod = ExecutePeriod.DEPLOY, catalog = CatalogEnum.UIFACTORY, subCatalog = SubCatalogEnum.UF_BTNOREVENT, description = "定义表体字段编辑前事件", solution = "在xml中使用nc.ui.pubapp.uif2app.event.card.CardBodyBeforeEditEvent  ", coder = "zhongcha", relatedIssueId = "185")
public class TestCase00185 extends AbstractXmlRuleDefinition {
    ...
    protected IRuleExecuteResult processScriptRules(IRuleExecuteContext ruleExecContext, List<XmlResource> resources)
        ...
        for (XmlResource xmlResource : resources) {
            ...
            for (Element element : eventElement) {
                ...
                if (null != e && MmUIFactoryConstants.BODY_BEFORE_EDIT_EVENT.equals(e.getAttribute("value"))) {
                    flag = true;
                    break;
                }
            }
        ...
        if (!funNodeNames.isEmpty()) {
            result.addResultElement(ruleExecContext.getBusinessComponent().getDisplayBusiCompName(), String.format(
                    "功能节点编码:%s 的配置文件没有实现表体字段编辑前事件[%s]\n", funNodeNames, MmUIFactoryConstants.BODY_BEFORE_EDIT_EVENT));
        }
        return result;
    }
}
