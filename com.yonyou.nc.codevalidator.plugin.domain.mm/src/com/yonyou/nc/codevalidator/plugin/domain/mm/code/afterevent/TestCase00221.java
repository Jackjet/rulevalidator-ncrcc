...
/**
 * 表体行编缉后事件需要为单据行的pk_group、pk_org、pk_org_v等字段设置默认值
 * 
 * @since 6.0
 * @version 2013-11-4 上午10:53:17
 * @author zhongcha
 */
@RuleDefinition(executePeriod = ExecutePeriod.CHECKOUT, catalog = CatalogEnum.JAVACODE, subCatalog = SubCatalogEnum.JC_CODECRITERION, description = "表体行编缉后事件需要为单据行的pk_group、pk_org、pk_org_v等字段设置默认值   。", relatedIssueId = "221", coder = "zhongcha", solution = "在表体行编缉后事件处理类中为单据行的pk_group、pk_org、pk_org_v等字段设置默认值 。")
public class TestCase00221 extends AbstractJavaQueryRuleDefinition {
        ...
        try {
            ...
            if (MMValueCheck.isNotEmpty(errorMapList)) {
                for (String key : errorMapList.keySet()) {
                    errorContxt.append("该组件中名为" + key + "的在表体行编缉后事件处理类中没有为单据行字段" + errorMapList.get(key) + "设置默认值。\n");
                }
            }
            if (!isExsitCorrectHandler) {
                errorContxt.append("该组件中不存在类名后缀为BodyAfterRowEditHandler的表体行编缉后事件处理类。\n");
            }
    ...
    private static class ScanRule extends AbstractJavaRule {
        ...
        /**
         * 根据该被扫描类实现的接口中的事件类型是否为nc.ui.pubapp.uif2app.event.card.CardBodyAfterRowEditEvent
         */
        @Override
