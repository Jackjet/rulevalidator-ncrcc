...
@RuleDefinition(executePeriod = ExecutePeriod.CHECKOUT, catalog = CatalogEnum.JAVACODE, subCatalog = SubCatalogEnum.JC_CODECRITERION, specialParamDefine = {
    "检查规则"
}, description = "更新BP需要判断单据状态的检查规则(若没有参数时，检查默认场景：更新BP命名格式为后缀是UpdateBP，规则类为BillUpdateStatusCheckRule。有参数，若该组件下只有一个节点，参数直接为规则（如：DomUpdateStatusCheckRule）,若存在多个节点，则参数形如dmo@CheckDmoStatusRule#BillUpdateStatusCheckRule（代表：节点编码为dmo下的BP类检测CheckDmoStatusRule和BillUpdateStatusCheckRule），指定节点下只检查指定的规则，组件中若还存在其他节点的BP，执行默认的或者参数中不带节点的其他规则的检查。)  ", relatedIssueId = "276", coder = "zhongcha", solution = "在前规则中添加判断单据状态的规则。", memo = "参数可空。存在多个参数请用，隔开。若该组件下只有一个节点，参数内容为需检查的规则（如：DomUpdateStatusCheckRule）,若存在多个节点，则参数形如dmo@CheckDmoStatusRule#BillUpdateStatusCheckRule（代表：节点编码为dmo下的BP类检测CheckDmoStatusRule和BillUpdateStatusCheckRule），指定节点下只检查指定的规则，组件中若还存在其他节点的BP，执行默认的或者参数中不带节点的其他规则的检查。) ")
public class TestCase00276 extends AbstractJavaQueryRuleDefinition {
    ...
    protected IRuleExecuteResult processJavaRules(IRuleExecuteContext ruleExecContext, List<JavaClassResource> resources)
        ...
        nodes.remove(TestCase00276.COM_PARAM);
                ...
                if (fileName.endsWith("UpdateBP")) {
                    ...
                    if (!isExistNode) {
                        ...
                        if (!package2BPClzs.keySet().contains(filePackage)
                            ...
                            package2BPClzs.put(filePackage, javaClassResource);
                            clz2rules.putAll(fileName, node2rules.get(TestCase00276.COM_PARAM));
                        }
                    }
                }
            }
    ...
    public MapList<String, String> getNode2Rules(String[] parameters) {
        ...
        if (MMValueCheck.isEmpty(parameters)) {
            node2rules.put(TestCase00276.COM_PARAM, "BillUpdateStatusCheckRule");
            return node2rules;
        }
        for (String parameter : parameters) {
            if (parameter.indexOf("@") <= 0) {
                ...
                for (String rule : rules) {
                    node2rules.put(TestCase00276.COM_PARAM, rule);
                }
            }
            else {
        ...
        if (MMValueCheck.isEmpty(node2rules.get(TestCase00276.COM_PARAM))) {
            node2rules.put(TestCase00276.COM_PARAM, "BillUpdateStatusCheckRule");
        }
        return node2rules;
    }
    ...
    private static class ScanUpdateBPRule extends AbstractJavaRule {
        ...
        /**
         * 扫描方法，若为addBeforeRule,则判断是否存在要检查的rule
         */
        @Override
