 ...
 * @version 1.0.0.0
 */
@RuleDefinition(executePeriod = ExecutePeriod.DEPLOY, catalog = CatalogEnum.UIFACTORY, subCatalog = SubCatalogEnum.UF_BTNOREVENT, description = "检查单据是否支持了主组织改变事件", solution = "检查单据是否支持了主组织改变事件",  coder = "lijbe", relatedIssueId = "180")
public class TestCase00180 extends AbstractXmlRuleDefinition {
    ...
    protected IRuleExecuteResult processScriptRules(IRuleExecuteContext ruleExecContext, List<XmlResource> resources)
        ...
        for (XmlResource xmlResource : resources) {
            ...
            for (Element element : eventElement) {
                ...
                if (null != e && MmUIFactoryConstants.ORG_CHANGED_EVENT.equals(e.getAttribute("value"))) {
                    flag = true;
        ...
        if (!funNodeNames.isEmpty()) {
            ...
            result.addResultElement(ruleExecContext.getBusinessComponent().getDisplayBusiCompName(), String.format(
                    "功能节点编码:%s 的配置文件没有实现主组织改变事件[%s]\n", funNodeNames, MmUIFactoryConstants.ORG_CHANGED_EVENT));
        }
        return result;
    }
