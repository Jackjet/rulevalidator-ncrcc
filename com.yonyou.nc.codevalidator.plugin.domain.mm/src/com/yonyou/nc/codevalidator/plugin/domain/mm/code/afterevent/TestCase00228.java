...
/**
 * 业务单位的编缉后事件中必须处理换算率 1,先按照关键字"*unitHandler.java",
...
@RuleDefinition(executeLayer = ExecuteLayer.BUSICOMP, executePeriod = ExecutePeriod.DEPLOY,
		...
		description = "业务单位的编缉后事件中必须处理换算率 ", relatedIssueId = "228", coder = "lijbe",
		solution = "1.通过编辑后事件类型找到事件监听处理类;2.根据关键字符*UnitHandler或者*unitHandler【单位编辑事件命名以其结尾】找到编辑事件处理类;"
				+ "2.分析afterEdit中是否调用了【nc.ui.mmf.busi.measure.handler.AssMeasureHandler.afterEdit方法】")
public class TestCase00228 extends AbstractXmlRuleDefinition {
	...
	private void checkConcreteHandler(JavaClassResource javaClassResource) {
		...
		try {
			...
			visitor.visit(compilationUnit, null);
			if (!visitor.isVchagerate) {
				noteBuilder
						.append(String
								.format("业务单位编辑事件处理类【%s】编辑后事件中没有处理换算率,即没有调用【nc.ui.mmf.busi.measure.handler.AssMeasureHandler.afterEdit方法】进行处理.\n",
										javaClassResource.getJavaCodeClassName()));
			}
		} catch (ParseException e) {
			Logger.error(e.getMessage(), e);
	...
	private class ConcreteEventHandlerVisitorAdapter extends VoidVisitorAdapter<Void> {
		...
		/**
		 * isVchagerate是否处理了换算率
		 */
		boolean isVchagerate = false;
		...
		public void visit(MethodDeclaration n, Void arg) {
			...
			for (Statement stmt : stmts) {
				...
				if (stmtStr.contains("new AssMeasureHandler") && !hasUnit) {
					String[] mhs = stmtStr.split(" ");
					variable = mhs[1];
					hasUnit = true;
				}
				if (stmtStr.contains(variable + ".afterEdit") && hasUnit) {
					this.isVchagerate = true;
				}
