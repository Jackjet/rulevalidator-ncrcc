 ...
 * @version 1.0.0.0
 */
@RuleDefinition(executeLayer = ExecuteLayer.BUSICOMP, executePeriod = ExecutePeriod.COMPILE, catalog = CatalogEnum.JAVACODE, subCatalog = SubCatalogEnum.JC_CODECRITERION, description = "后规则中是否进行了单据号的惟一性检查 【单据可选】", relatedIssueId = "272", coder = "lijbe", solution = "通过关键字符*InsertBP取bs包下的*InsertBP.java文件;再判断BP是否引用了关键字符nc.bs.pubapp.pub.rule.BillCodeCheckRule;注意添加后规则的方法名为addAfterRule.")
            ...
            VoidVisitorAdapter<Void> visitor = new VoidVisitorAdapter<Void>() {
                ...
                private void appendMsg(String methodName) {
                    noteBuilder.append(String.format(
                            "方法【%s】没有添加单据号唯一性校验规则【nc.bs.pubapp.pub.rule.BillCodeCheckRule】！\n",
