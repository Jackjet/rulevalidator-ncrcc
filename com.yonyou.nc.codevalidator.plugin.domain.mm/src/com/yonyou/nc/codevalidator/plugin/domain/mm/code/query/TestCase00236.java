...
/**
 * 如果物料作为查询条件，则物料ID、物料编码、物料名称需要进行参照过滤
 * 
 * @author qiaoyanga
 * @since V1.0
 * @version 1.0.0.0
 */
@RuleDefinition(executePeriod = ExecutePeriod.DEPLOY, catalog = CatalogEnum.JAVACODE, subCatalog = SubCatalogEnum.JC_CODEDESIGN, description = "如果物料作为查询条件，则物料ID、物料编码、物料名称需要进行参照过滤 ", relatedIssueId = "236", coder = "qiaoyanga", solution = "配置节点号参数，将通过节点号找到对应的配置文件，通过配置文件找到查询初试化类，判断类中是否有物料参照过滤类‘nc.ui.mmf.busi.query.reffilter.QMarterialFilter’")
public class TestCase00236 extends AbstractXmlRuleDefinition {
    @Override
    ...
    protected IRuleExecuteResult processScriptRules(IRuleExecuteContext ruleExecContext, List<XmlResource> resources)
        ...
        for (XmlResource xmlResource : resources) {
            ...
            try {
                ...
                if (!isImport) {
                    result.addResultElement(javaClassResource.getJavaCodeClassName(), "查询初试化类中物料需要进行参照过滤 \n");
                    continue;
                }
            }
    ...
    private static class QryCondDLGInitializer extends VoidVisitorAdapter<Void> {
        ...
        public void visit(ImportDeclaration n, Void arg) {
            ...
            if ("nc.ui.mmf.busi.query.reffilter.QMarterialFilter".equals(importClazz)) {
                this.isImport = true;
                return;
            }
        }
