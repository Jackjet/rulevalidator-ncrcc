...
/**
 * 模块: TranslateToInformix.java 描述: 将SqlServer语句翻译到Informix语句
 */
...
public class TranslateToInformix extends TranslatorObject {
	...
	/**
	 * 处理连接更新 创建日期：(2004-11-05 8:38:35)
	 * 
	...
	private StringBuffer translateSelect(String[] asSqlWords) throws Exception {
		...
		while (iOffSet < asSqlWords.length) {
			...
			// 处理count(1)->count(*)
			if (iOffSet + 3 < asSqlWords.length
					&& asSqlWords[iOffSet].equalsIgnoreCase("count")
					&& asSqlWords[iOffSet + 1].equals("(")
					&& (asSqlWords[iOffSet + 2].equals("1") || asSqlWords[iOffSet + 2]
							.equals("0"))
	...
	/**
	 * 返回字符串数组asSqlWords中从start到end处的字符串。 创建日期：(2004-11-03 19:50:50)
	 */
	...
	/**
	 * 此处插入方法说明。 创建日期：(2004-11-03 19:50:50)
	 */
	...
	public boolean isFunctionName(String sWord, String nextWord) {
		...
		if ((sWord.equalsIgnoreCase("square")
				...
				|| sWord.equalsIgnoreCase("round") || sWord
	...
	/**
	 * 转换Convert函数 参数: asWords 函数子句 规则: Convert(datatype,col) -> cast(col as
	 * datatype)
	 */
	...
	/*
	 * round(参数1,参数2,参数3) 规则: round(参数1,参数2,参数3)->trunc(参数1,参数2)
	...
	/**
	 * 转换Square函数 参数: asWords 函数子句 规则: square(f)->f*f
	 */
			...
			if (asWords[iOffSet].equalsIgnoreCase("set")) {
				...
				while (iOffSet < asWords.length
					...
					if (sTableAlias != null && sTableAlias.trim().length() > 0
						...
						asWords[iOffSet] = sTableName
								+ asWords[iOffSet].substring(i);
					}
	...
	private String[][] fun_inf = { { "len", "length" },
			{ "substring", "substr" }, { "lower", "lower" },
			{ "upper", "upper" }, { "isnull", "nvl" }, { "rtrim", "trim" },
			{ "ltrim", "trim" }
