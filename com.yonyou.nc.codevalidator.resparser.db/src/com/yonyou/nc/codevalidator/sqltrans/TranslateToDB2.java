...
/**
 * 模块: TranslateToDB2.java 描述: 将SqlServer语句翻译到DB2语句 作者: cf
 */

public class TranslateToDB2 extends TranslatorObject {

	// 函数对照表,列出sqlServer函数与DB2函数的对应关系,
	private String[][] fun_db2 = { { "len", "length" },
			{ "substring", "substr" }, { "lower", "lcase" },
	...
	private StringBuffer translateDelete(String[] sqlWords) throws Exception {
		...
		while (iOffSet < sqlWords.length) {
			...
			// 处理函数
			if ((sWord.equalsIgnoreCase("square") || sWord
			...
			// 处理PI()
			if (sWord.equalsIgnoreCase("PI")
					&& sqlWords[iOffSet + 1].equals("(")
					&& sqlWords[iOffSet + 2].equals(")")) {
			...
			if (sPreWord.equalsIgnoreCase("like")) {
				if (sWord.indexOf("[") > 0 && sWord.indexOf("]") > 0) {
					...
					sSql = sSql
							+ sWord.substring(sWord.indexOf("[") + 1,
									sWord.indexOf("]"));
	...
	private void translateFunction() {
		...
		while (iOffSet < m_asSqlWords.length) {
			...
			// 找到函数
			if (iOffSet > 1
					&& m_asSqlWords[iOffSet - 2].equalsIgnoreCase("convert")
	...
	private StringBuffer translateSelect(String[] asSqlWords) throws Exception {
		...
		while (iOffSet < asSqlWords.length) {
			...
			// 处理优化关键字
			if (iOffSet < asSqlWords.length && iOffSet + 5 < asSqlWords.length
			...
			// }
			// }
			if (iOffSet < asSqlWords.length && iOffSet > 0
					&& asSqlWords[iOffSet].equalsIgnoreCase("select")
					&& asSqlWords[iOffSet - 1].equals("(")) {
			...
			if (iOffSet < asSqlWords.length - 1
					&& asSqlWords[iOffSet].equalsIgnoreCase("+")
					&& (asSqlWords[iOffSet + 1].indexOf("'") > -1 || asSqlWords[iOffSet - 1]
							.indexOf("'") > -1)) {
				m_sbDestinationSql.append("||");
				iOffSet++;
			...
			if (asSqlWords[iOffSet].equalsIgnoreCase("null") && iOffSet > 0
					&& iOffSet < 2 && !asSqlWords[iOffSet - 1].equals("=")
					&& !asSqlWords[iOffSet - 1].equals("(")) {
				...
				iOffSet++;
					...
					&& iOffSet < asSqlWords.length && iOffSet > 0 && hasWhere
		...
		if (m_sbDestinationSql != null) {
			m_sbDestinationSql.replace(0, m_sbDestinationSql.toString()
					.length(), m_sbDestinationSql.toString().trim());
		}
	...
	/**
	 * 此处插入方法说明。 创建日期：(2002-4-3 19:50:50)
	 */
	public TransUnit dealFunction(String[] asSqlWords, String sWord, int iOffSet)
			throws Exception {
	 ...
	 * 此处插入方法说明。 创建日期：(2002-4-3 19:50:50)
	 */
	public TransUnit dealSelect(String[] asSqlWords, String sWord, int iOffSet)
			throws Exception {
	...
	public boolean isFunctionName(String sWord, String nextWord) {
		...
		if ((// (sWord.equalsIgnoreCase("left")&&
				// !nextWord.equalsIgnoreCase("outer")) /** 1 **/
				// || (sWord.equalsIgnoreCase("right")&&
				// !nextWord.equalsIgnoreCase("outer")) /** 2 **/
	...
	/**
	 * 在表达式中等号后边寻找是否有指定的表名或表的别名以外的表名或表的别名 如果有，返回true,否则，返回false Author:ljq
	 * 
	 * @return boolean
	 * @param s
	 *            java.lang.String表达式
	 ...
	 *            java.lang.String表别名
	 */
	public boolean searchHaveOther(String s, String sTableName,
			String sTableAlias) {
		...
		if (s.lastIndexOf(".") > s.indexOf("=")) // 张森2001.3.17 改
		{
			...
			while (i0 < sDotString.length() - 1) {
				...
				if (sDotString.substring(i0, i0 + 1).equals("+")
						...
						|| sDotString.substring(i0, i0 + 1).equals("*")
						|| sDotString.substring(i0, i0 + 1).equals("/")
						|| sDotString.substring(i0, i0 + 1).equals("%") // 张森2001.3.17
																		// 加
						|| (sDotString.substring(i0, i0 + 1).equals("|") && sDotString
								.substring(i0 + 1, i0 + 2).equals("|")) // 判断字符连接
						|| sDotString.substring(i0, i0 + 1).equals("(") // 判断函数
						|| sDotString.substring(i0, i0 + 1).equals(")")) {
					if (sDotString.substring(i0, i0 + 1).equals("(")) // 张森2001.3.17
																		// 加,如果遇到括号，则跳过
					{
						iStart = i0;
					} else {
						...
						if (s0.indexOf(".") > 0) {
							...
							if (!s0.substring(iStart, iEnd).equalsIgnoreCase(
									sTableName)
									&& !s0.substring(iStart, iEnd)
											.equalsIgnoreCase(sTableAlias)) {
								bFlg = true;
								break;

							} else {
								// 若截取字符串等于目标表名且等于目标表别名，//张森2001.3.17 加
								if (sDotString.substring(i0, i0 + 1)
										.equals("|") // 如果遇到 "||"
										&& sDotString.substring(i0 + 1, i0 + 2)
												.equals("|")) {
									iStart = i0 + 1;
								} else {
									iStart = i0;
			...
			if (!bFlg) {
				...
				if (s0.indexOf(".") > 0) {
					 ...
					 */
					if (!s0.substring(0, iEnd).equalsIgnoreCase(sTableName) // 张森2001.3.17
																			// 改
							&& !s0.substring(0, iEnd).equalsIgnoreCase(
									sTableAlias)) {
						bFlg = true;
					}
				}
	...
	/**
	 * 在表达式中等号后边]寻找指定的表名或表的别名 如果有，返回true,否则，返回false Author:ljq
	 * 
	 * @return boolean
	 * @param s
	 *            java.lang.String表达式
	...
	/**
	 * 转换Square函数 参数: asWords 函数子句 规则: square(f)->power(f,2)
	 */
		...
		if (col.equalsIgnoreCase("null"))
			...
			if (isDateType(dataType)) {
				...
				if (oldDataType.indexOf("(") > 0) {
					charLenCtrl = true;
					charLenth = oldDataType.substring(
							oldDataType.indexOf("(") + 1,
	...
	private void translateFunDateAdd(String[] asWords) {
		...
		try {
			...
			// 张时栋修改 2002-05-22
			if (theDate.toLowerCase().startsWith("getdate(")
	...
	private void translateFunDateDiff(String[] asWords) {
		...
		try {
			...
			// 张时栋修改 2002-06-12
			if (dateType != null
					&& (dateType.trim().equalsIgnoreCase("day")
							|| dateType.trim().equalsIgnoreCase("dd") || dateType
							.trim().equalsIgnoreCase("d"))) {

				if (theStart.toLowerCase().startsWith("getdate(")
				...
				if (theEnd.toLowerCase().startsWith("getdate(")
					...
					&& (dateType.trim().equalsIgnoreCase("month")
							|| dateType.trim().equalsIgnoreCase("mm") || dateType
				...
				if (theEnd.toLowerCase().startsWith("getdate(")
					...
					&& (dateType.trim().equalsIgnoreCase("year")
							|| dateType.trim().equalsIgnoreCase("yyyy") || dateType
	...
	private void translateFunRound(String[] asSqlWords) throws Exception {
		...
		if (commaCount == 0) {
			...
			if (tyle == 0) {
				...
				// + theLength + ") ";
				s = " floor(" + theNumber + "*(power(10.0," + theLength
						+ ")))/(power(10.0," + theLength + ")) ";
			}
		}
	...
	private void translateIFExists(String[] asSqlWords) {
		...
		int index = -1;
		String result = "";
		if ((index = isHasWord(asSqlWords, "drop")) > -1
				&& asSqlWords[index + 1].equalsIgnoreCase("table")) {
		...
		while (iOffSet < asWords.length - 1) {
			...
			if (asWords[iOffSet].equalsIgnoreCase("set")) {
				...
				while (iOffSet < asWords.length
					...
					if (asWords[iOffSet].equalsIgnoreCase("(")) // 左括号数，张森，2001。3。17
																// 加
					{
						leftCount++;
			...
			if (iOffSet < asWords.length
				...
				// TransUnit aTransUnit = dealCaseWhen(asWords,
				// asWords[iOffSet], iOffSet);
	...
	/**
	 * 转换Update语句 2002-07-22 暂时不用该函数
	 */
	private StringBuffer translateUpdate_old(String[] asSqlWords)
			throws Exception {
		...
		while (iOffSet < asWords.length) {
			...
			// 处理PI()
			if (asWords[iOffSet].equalsIgnoreCase("PI")
			...
			if (sPreWord.equalsIgnoreCase("like")) {
				if (asWords[iOffSet].indexOf("[") > 0
						&& asWords[iOffSet].indexOf("]") > 0) {
					sSql = "";
					sSql = sSql
							+ asWords[iOffSet].substring(0,
									asWords[iOffSet].indexOf("["));
					sSql = sSql
							+ asWords[iOffSet].substring(
									asWords[iOffSet].indexOf("[") + 1,
									asWords[iOffSet].indexOf("]"));
					sSql = sSql
							+ asWords[iOffSet].substring(asWords[iOffSet]
									.indexOf("]") + 1);
					m_sbDestinationSql.append(" " + sSql);
					iOffSet++;
				}
			...
			if (asWords[iOffSet].equalsIgnoreCase("from") && !m_bSubSelect) {
				...
				if (iOffSet < asWords.length) {
					if (!asWords[iOffSet].equals(",")
							&& !asWords[iOffSet].equals(")")
							&& !asWords[iOffSet].equals("]")
							&& !sPreWord.equals("(") && !sPreWord.equals("["))
						m_sbDestinationSql.append(" ");
					m_sbDestinationSql.append(asWords[iOffSet]);
		...
		if (m_bUpdateFrom) {
			if (m_sbDestinationSql.toString().endsWith(";")) {
				m_sbDestinationSql.replace(m_sbDestinationSql.toString()
						.length() - 1,
						m_sbDestinationSql.toString().length() - 1, ")");
			} else {
				m_sbDestinationSql.append(")");
			}
		}
	 ...
	 * 转换Update语句
	 */
	private StringBuffer translateUpdateII(String[] asSqlWords)
			throws Exception {
	...
	private void translateFunCast(String[] asWords) throws Exception {
		Logger.setThreadState("nc.bs.mw.sqltrans.TranslateToDB2.translateFunCast");
		...
		Logger.setThreadState("nc.bs.mw.sqltrans.TranslateToOracle.TranslateToDB2 Over");
	}
}