...
public class KeyValueComboBox extends Composite {
	 ...
	 * @see Widget#getStyle()
	 */
	public KeyValueComboBox(Composite parent, int style) {
		...
		listener = new Listener() {
			public void handleEvent(Event event) {
				...
				if (KeyValueComboBox.this == event.widget) {
					comboEvent(event);
					return;
				}
		...
		filter = new Listener() {
			public void handleEvent(Event event) {
				...
				if (shell == KeyValueComboBox.this.getShell()) {
					handleFocus(SWT.FocusOut);
				}
			}
		};
		...
		int[] textEvents = { SWT.KeyDown, SWT.KeyUp, SWT.MenuDetect,
				SWT.Modify, SWT.MouseDown, SWT.MouseUp, SWT.Traverse,
				SWT.FocusIn };
	...
	void listEvent(Event event) {
		...
		case SWT.Selection: {
			...
			if (index == -1)
				return;
			KeyValue kv = hmKeyValues.get(index + "");
			if (kv != null) {
				text.setText(kv.getValue());
			} else {
	...
	private HashMap<String, KeyValue> hmKeyValues = new HashMap<String, KeyValue>();

	public void setItems(KeyValue[] keyValues) {
		hmKeyValues.clear();
		// list.setItems(new String[]{});
		int count = keyValues == null ? 0 : keyValues.length;
		String[] items = new String[count];
		for (int i = 0; i < count; i++) {
			if (!keyValues[i].getKey().equals("")) {
				items[i] = keyValues[i].getKey() + "  ->  "
						+ keyValues[i].getValue();
			} else {
				items[i] = "";
			}
			hmKeyValues.put(i + "", keyValues[i]);
		}
		setItems(items);
	}
